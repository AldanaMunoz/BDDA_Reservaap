...
const dbName = process.env.DB_NAME || 'reservapp';
...
async function resetAndLoadData() {
  try {
    ...
    await db.query('DELETE FROM comidas_planificacion_semanal');
    console.log('   ✅ Planificaciones semanales eliminadas');

    await db.query('DELETE FROM planificaciones_semanales');
    console.log('   ✅ Planificaciones eliminadas');

    await db.query('DELETE FROM temporadas');
    console.log('   ✅ Temporadas eliminadas');

    await db.query('DELETE FROM comida_tiene_restriccion');
    console.log('   ✅ Restricciones de comidas eliminadas');

    await db.query('DELETE FROM comidas');
    console.log('   ✅ Comidas eliminadas');

    await db.query('DELETE FROM usuario_restricciones');
    console.log('   ✅ Restricciones de usuarios eliminadas');

    await db.query('DELETE FROM usuarios_roles');
    console.log('   ✅ Roles de usuarios eliminados');

    await db.query('DELETE FROM empleados');
    ...
    // ==========================================
    // 3. CREAR USUARIOS EMPLEADOS
    // ==========================================
    console.log('\n📋 PASO 3: Creando usuarios empleados...');
    ...
    // Obtener ID de restricción Sin TACC
    ...
    planificacionSemanal.forEach(plan => {
      comidasUnicas.add(JSON.stringify({ nombre: plan.entrada, tipo: tipoEntrada }));
      comidasUnicas.add(JSON.stringify({ nombre: plan.principal, tipo: tipoPrincipal }));
      comidasUnicas.add(JSON.stringify({ nombre: plan.vegetariano, tipo: tipoVegetariano }));
      comidasUnicas.add(JSON.stringify({ nombre: plan.alternativo, tipo: tipoAlternativo }));
    });
    ...
    for (const comidaStr of comidasUnicas) {
      ...
      if (platosSinTacc.includes(comida.nombre)) {
        await db.query(
          ...
          [result.insertId, restriccionSinTacc]
        );
      }
    }
    ...
    // 5. CREAR TEMPORADA
    // ==========================================
    console.log('\n📋 PASO 5: Creando temporada Primavera 2025...');
    ...
    // ==========================================
    // 6. CREAR PLANIFICACIÓN SEMANAL
    // ==========================================
    console.log('\n📋 PASO 6: Creando planificación semanal...');
    ...
    // 7. CREAR RESERVAS
    // ==========================================
    console.log('\n📋 PASO 7: Generando reservas realistas...');
    ...
    console.log('\n📊 Resumen Final:');
    console.log(`   - 1 usuario administrador (admin@reservapp.com / Admin123!)`);
    console.log(`   - ${usuarios.length} empleados (empleado1@reservapp.com hasta empleado${usuarios.length}@reservapp.com / Test123!)`);
    console.log(`   - ${Math.round(usuarios.length * 0.8)} internos (80%), ${Math.round(usuarios.length * 0.2)} externos (20%)`);
